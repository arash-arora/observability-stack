...
export default function EvaluationDetailView({
  ...
  const traceObservations = result.metadata_json?.trace_observations;
        ...
        <div className="grid gap-6 md:grid-cols-2 border rounded-lg p-6 bg-muted/5">
          ...
          {!isAgentic && showInput && (
            <div className="col-span-2">
              <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Node Input</h4>
          ...
          {!isAgentic && showOutput && (
            <div className="col-span-2">
              <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Node Output</h4>
              <pre className="p-4 rounded-md bg-muted/50 border font-mono text-sm whitespace-pre-wrap overflow-x-auto">
          ...
          {isAgentic && (
            <div className="col-span-2 space-y-4">
              <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Complete Trace</h4>
                      ...
                      <div key={obs.id || index} className="border rounded-lg bg-background shadow-sm overflow-hidden">
                        ...
                        <div className="flex items-center justify-between p-3 bg-muted/20 border-b">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant="outline" className={badgeColor + " font-semibold capitalize text-[10px]"}>
                              {obs.type || "node"}
                            </Badge>
                            <span className="font-semibold text-sm text-foreground">{obs.name}</span>
                          </div>
                          {obs.start_time && (
                            <span className="text-[11px] text-muted-foreground">
                              {format(new Date(obs.start_time), "HH:mm:ss.SSS")}
                            </span>
                          )}
                        </div>
                        {/* Content */}
                        <div className="p-3 space-y-3 bg-card text-xs">
                          {obs.input && (
                            <div className="space-y-1">
                              <span className="font-semibold text-muted-foreground block text-[10px] uppercase tracking-wider">Input</span>
                              <pre className="p-2 rounded bg-muted/40 font-mono text-[11px] whitespace-pre-wrap overflow-x-auto max-h-40 border">
                          ...
                          {obs.output && (
                            <div className="space-y-1">
                              <span className="font-semibold text-muted-foreground block text-[10px] uppercase tracking-wider">Output</span>
                              <pre className="p-2 rounded bg-muted/40 font-mono text-[11px] whitespace-pre-wrap overflow-x-auto max-h-40 border">
          ...
          {showContext && (
            <div className="col-span-2">
              <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Node Context</h4>
                      ...
                      {workflowDetails.tools.map((tool: string) => {
                        ...
                        return (
                          <div key={tool} className="p-3 rounded-lg border bg-background/50 flex flex-col gap-1 shadow-sm">
                            <div className="flex items-center gap-2">
                              <Badge variant="outline" className="border-primary/30 text-primary bg-secondary/5 hover:bg-secondary/5 font-mono text-[11px]">
                                {name}
                              </Badge>
                            </div>
                            {desc && (
